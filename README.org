...
together in a table.  In datomic we define a bunch of fields that dont
...
In datomic we dont have tables, just a bunch of maps.  So we dont have
...
query.  This is a very important idea which we will revist later to
re-inforce.
...
clauses.  Where clauses are implicitly ANDed together.  So both
...
database and regular old clojure data structures that dont really
support references.
...
attribute (field) whos value is: ~"ftravers"~.  Now lets do some magic
